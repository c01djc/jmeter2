...
val baseFolder = "jmeter2-${rootProject.version}"
...
fun CrLfSpec.binaryLayout() = copySpec {
    ...
    into(baseFolder) {
        ...
        from(rootDir) {
            ...
            // JMeter2: only ship jmeter2 launchers (not upstream jmeter/server/mirror helpers)
            exclude("bin/jmeter")
            exclude("bin/jmeter.bat")
            exclude("bin/jmeter.sh")
            exclude("bin/jmeterw.cmd")
            exclude("bin/jmeter-server")
            exclude("bin/jmeter-server.bat")
            exclude("bin/jmeter-n.cmd")
            exclude("bin/jmeter-n-r.cmd")
            exclude("bin/jmeter-t.cmd")
            exclude("bin/shutdown.cmd")
            exclude("bin/shutdown.sh")
            exclude("bin/stoptest.cmd")
            exclude("bin/stoptest.sh")
            exclude("bin/heapdump.cmd")
            exclude("bin/heapdump.sh")
            exclude("bin/threaddump.cmd")
            exclude("bin/threaddump.sh")
            exclude("bin/mirror-server")
            exclude("bin/mirror-server.cmd")
            exclude("bin/mirror-server.sh")
            exclude("bin/create-rmi-keystore.bat")
            exclude("bin/create-rmi-keystore.sh")
            include("bin/**")
            include("lib/ext/**")
            include("lib/junit/**")
            include("extras/**")
...
for (type in listOf("binary", "source")) {
    ...
    for (archive in listOf(Zip::class, Tar::class)) {
        ...
        val archiveTask = tasks.register(taskName, archive) {
            ...
            // JMeter2 binary/source archives unpack to jmeter2-<version>/
            archiveBaseName.set("jmeter2-${rootProject.version}${if (type == "source") "_src" else ""}")
