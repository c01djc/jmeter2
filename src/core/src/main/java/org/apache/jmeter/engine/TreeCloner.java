...
public class TreeCloner implements HashTreeTraverser {
    ...
    private static final boolean SHARE_DISABLED =
            JMeterUtils.getPropDefault("jmeter.tree_clone.share_disabled", true); // $NON-NLS-1$
    ...
    protected Object addNodeToTree(Object node) {
        // Disabled elements are not executed; sharing the original instance across VUs
        // avoids cloning large disabled subtrees (memory win on big plans).
        if (SHARE_DISABLED && node instanceof TestElement && !((TestElement) node).isEnabled()) {
