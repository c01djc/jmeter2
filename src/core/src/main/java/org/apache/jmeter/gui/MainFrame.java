...
public class MainFrame extends JFrame implements TestStateListener, Remoteable, DropTargetListener, Clearable, ActionListener {
    ...
    private static final String DEFAULT_APP_NAME = org.apache.jmeter.JMeter2Branding.APP_NAME;
    ...
    public void setMainPanel(JComponent comp) {
        ...
        // Force layout refresh: long sessions / LAF / HiDPI / off-EDT visualizer updates
        // can leave the detail panel clipped or blank until the next resize.
        if (comp != null) {
            comp.invalidate();
            comp.revalidate();
        }
    ...
    private void init() { // WARNING: called from ctor so must not be overridden (i.e. must be private or final)
        ...
        java.util.List<java.awt.Image> icons = org.apache.jmeter.JMeter2Branding.loadAppIconImages();
        if (!icons.isEmpty()) {
            setIconImages(icons);
        }
    ...
    private Component createToolBar() {
        ...
        // Glue starts the status area; must survive toolbar rebuild on language change
        ...
        // Refresh status tooltips when locale changes
        JMeterUtils.addLocaleChangeListener(event -> refreshStatusTooltips());
        return toolPanel;
    }

    private void refreshStatusTooltips() {
        testTimeDuration.setToolTipText(JMeterUtils.getResString("duration_tooltip")); // $NON-NLS-1$
        activeAndTotalThreads.setToolTipText(JMeterUtils.getResString("active_total_threads_tooltip")); // $NON-NLS-1$
        warnIndicator.setToolTipText(JMeterUtils.getResString("error_indicator_tooltip")); // $NON-NLS-1$
    }
