...
public class JSyntaxTextArea extends RSyntaxTextArea {
    ...
    /**
     * Apply XML Theme to syntax text area and keep font in sync with UI zoom scale.
     * Always sets an absolute {@code baseSize * scale} so request/body editors zoom
     * with the rest of the UI (avoids partial zoom / stuck small fonts).
     *
     * @param jSyntaxTextArea target area
     */
    ...
    /**
     * Apply global zoom scale to this syntax area (syntax scheme follows via {@code setFont}).
     */
    private static void applyZoomFont(JSyntaxTextArea area) {
        Font current = area.getFont();
        String family = USER_FONT_FAMILY != null
                ? USER_FONT_FAMILY
                : (current != null ? current.getFamily() : Font.MONOSPACED);
        float baseSize = USER_FONT_SIZE > 0 ? USER_FONT_SIZE : DEFAULT_BASE_FONT_SIZE;
        float scale = JMeterUIDefaults.INSTANCE.getScale();
        float size = Math.max(1f, baseSize * scale);
        int style = current != null ? current.getStyle() : Font.PLAIN;
        Font font = JMeterUIDefaults.createFont(family, style, Math.round(size)).deriveFont(size);
        area.setFont(font);
    }
