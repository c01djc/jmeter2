...
public final class GuiPackage implements LocaleChangeListener, HistoryListener {
    ...
    */
    ...
    public void localeChanged(LocaleChangeEvent event) {
        ...
        // Tree node names are stored as plain strings (often the label from the
        // locale used when the element was created). Retranslate defaults so
        // Options → Language updates Test Plan / Thread Group / listeners etc.
    ...
    /**
     * If a test element still uses a localized default name from another locale,
     * rename it to the current locale's default. User-renamed nodes are left alone.
     */
    private void retranslateDefaultElementNames() {
        JMeterTreeNode root = (JMeterTreeNode) treeModel.getRoot();
        if (root == null) {
            return;
        }
        retranslateDefaultElementNames(root);
    }

    private void retranslateDefaultElementNames(JMeterTreeNode node) {
        TestElement element = node.getTestElement();
        if (element != null) {
            try {
                JMeterGUIComponent gui = getGui(element);
                if (gui != null) {
                    String labelKey = gui.getLabelResource();
                    String name = element.getName();
                    if (labelKey != null && name != null
                            && isLocalizedDefaultName(name, labelKey)) {
                        String newName = JMeterUtils.getResString(labelKey);
                        if (!name.equals(newName)
                                && !newName.startsWith(JMeterUtils.RES_KEY_PFX)) {
                ...
                log.debug("Could not retranslate name for {}", element, ex);
            }
        }
        for (int i = 0; i < node.getChildCount(); i++) {
            retranslateDefaultElementNames((JMeterTreeNode) node.getChildAt(i));
        }
    }

    /**
     * @return true if {@code name} equals this component's default label in any
     *         supported UI language (or the previous locale)
     */
    private static boolean isLocalizedDefaultName(String name, String labelKey) {
        Locale previous = JMeterUtils.getPreviousLocale();
        if (previous != null && name.equals(JMeterUtils.getResString(labelKey, previous))) {
            return true;
        }
        if (name.equals(JMeterUtils.getResString(labelKey, Locale.ENGLISH))) {
            return true;
        }
        for (String lang : org.apache.jmeter.gui.util.JMeterMenuBar.getLanguages()) {
            Locale loc = toLocale(lang);
            if (name.equals(JMeterUtils.getResString(labelKey, loc))) {
                return true;
            }
        }
        return false;
    ...
    private static Locale toLocale(String locale) {
        ...
        if (sep > 0) {
            return new Locale(locale.substring(0, sep), locale.substring(sep + 1));
        }
