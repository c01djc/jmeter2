...
public abstract class SamplerResultTab implements ResultRenderer {
    ...
    private Component createResponseMetadataPanel() {
        ...
        bindZoomFont(stats, "TextPane.font"); // $NON-NLS-1$
    ...
    private JPanel createResponseDataPanel() {
        ...
        bindZoomFont(results, "TextArea.font"); // $NON-NLS-1$
    ...
    /**
     * Keep request/response text panes in sync with Options → Zoom In/Out.
     * Plain {@link JEditorPane}/{@link JTextPane} do not follow zoom by default,
     * and kerning tweaks can freeze a pre-zoom font size.
     */
    private static void bindZoomFont(javax.swing.text.JTextComponent component, String uiFontKey) {
        JFactory.withDynamic(component, c -> {
            Font font = UIManager.getFont(uiFontKey);
            if (font == null) {
            ...
            if (document instanceof StyledDocument) {
                ...
                Style def = styled.getStyle(StyleContext.DEFAULT_STYLE);
                if (def != null) {
